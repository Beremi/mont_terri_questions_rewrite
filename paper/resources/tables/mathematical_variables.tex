...
\(\uu=(u_x,u_y)\) &
\(\mathrm{m}\) &
Primary mechanical unknown and current OGS output field.  It describes deformation of
the continuum slice relative to the chosen numerical initial state.\\
Liquid pressure &
\(\pl\) &
\(\mathrm{Pa}\) &
Primary hydraulic unknown and current OGS output field.  In the Richards convention
used here, negative liquid pressure means positive suction relative to the
gas/reference pressure.\\
Temperature &
\(\TT\) &
\(\mathrm{K}\) &
Primary thermal unknown in the full TRM process and current OGS output field.  In the
...
the current OGS mesh field \texttt{k\_i\_rd}.\\
Porosity &
\(\por(\xx)\) &
-- &
...
\(S_e(\pc)\) &
-- &
Normalized saturation variable used by the van Genuchten retention and
...
Strain and stress &
\(\eps(\uu)\), \(\eps^{\mathrm{th}}(\TT)\), \(\sig\), \(\sig^{\mathrm{sw}}\) &
--, \(\mathrm{Pa}\) &
Mechanical fields derived from displacement, orthotropic elasticity, thermal strain,
saturation-dependent swelling stress, and Bishop/Biot pore-pressure coupling.\\
Swelling stress contribution &
\(\sig^{\mathrm{sw}}\), \(p_{\mathrm{sw}}\) &
--, \(\mathrm{Pa}\) &
Saturation-dependent stress contribution computed from the active
swelling-stress-rate law; \(p_{\mathrm{sw}}\) is the configured swelling pressure.\\
