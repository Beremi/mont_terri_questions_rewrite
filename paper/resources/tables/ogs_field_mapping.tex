...
Run-local VTU array \texttt{displacement}.  Mechanical observations need a
...
Run-local VTU array \texttt{pressure}.  Pressure measurements and RH-derived
...
Run-local VTU array \texttt{temperature}.  In the active parametrization it is
...
Run-local \texttt{MeshElement} field \texttt{k\_i\_rd} on the projected bulk mesh.
...
Run-local \texttt{MeshElement} field \texttt{n\_rd}; current field package is
spatially constant at \(0.105\). &
Run-local VTU array \texttt{porosity}.  Used with saturation to form
\(\theta=\por\Sl\).\\
\(\Sl\) &
...
primary variable. &
Run-local VTU array \texttt{saturation}.  Main state field for water-content
