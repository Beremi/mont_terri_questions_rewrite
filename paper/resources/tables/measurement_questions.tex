...
The available source data, generated reduction, and operator are sufficient for
current active use.\\
...
The available source data, email resources, and public sources do not provide the
numeric export needed for a hard residual.\\
\end{tabular}
...
{Which generated reduction defines the measurement positions?}
...
\mqPair{mqReady}{Ready}
{Which streams have available measured-data plots?}
...
{What must a row contain before an inverse run may use it?}
...
not enough.}
\mqPair{mqPolicy}{Policy}
{How should incomplete rows enter candidate comparisons?}
...
\mqPair{mqGate}{Gate}
{What inverse-problem failure does support uncertainty create?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqReady}{Ready}
{Which unknowns does the permeability likelihood constrain directly?}
...
\(\KK\) changes.}
\mqPair{mqGate}{Gate}
{What can bias the fitted permeability field if left unresolved?}
...
\(\KK(\xx)\).}
\mqPair{mqPolicy}{Policy}
{What is the minimum final-use policy for this stream?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqGate}{Gate}
{Which forward state does an NMR residual actually depend on?}
...
\mqPair{mqPolicy}{Policy}
{What nuisance parameters are needed for absolute NMR use?}
...
\mqPair{mqGate}{Gate}
{What data-leakage risk must be avoided?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqDiagnostic}{Diagnostic}
{What is the computational ERT observation operator?}
...
\mqPair{mqGate}{Gate}
{Why can dense ERT fields dominate an inverse problem incorrectly?}
...
\mqPair{mqGate}{Gate}
{Which ERT uncertainties are mathematically confounded?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqDiagnostic}{Diagnostic}
{What does the current Taupe/TDR residual retain and discard?}
...
\mqPair{mqGate}{Gate}
{What would be required for absolute Taupe/TDR likelihood use?}
...
\mqPair{mqGate}{Gate}
{What inverse-problem risk comes from using all Taupe bands independently?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqDiagnostic}{Diagnostic}
{Is RH a residual or a forcing parameter in the inverse problem?}
...
\mqPair{mqGate}{Gate}
{Why is RH boundary uncertainty crucial for permeability inversion?}
...
\mqPair{mqGate}{Gate}
{What is the minimum computational representation of \(p_E(t)\)?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqGate}{Gate}
{What would the pressure observation operator be?}
...
\mqPair{mqGate}{Gate}
{What computational error is most dangerous here?}
...
permeability.}
\mqPair{mqGate}{Gate}
{Why are pressure measurements valuable if completed?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqGate}{Gate}
{What OGS quantity could a crackmeter residual compare to?}
...
\mqPair{mqGate}{Gate}
{What missing metadata changes the sign of the inverse signal?}
...
\mqPair{mqGate}{Gate}
{How should crack evidence affect permeability candidates now?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqGate}{Gate}
{What mechanical observation operators are needed for full HM use?}
...
\mqPair{mqGate}{Gate}
{Can these HM streams calibrate permeability in the current frozen workflow?}
...
\mqPair{mqGate}{Gate}
{What would make HM residuals numerically defensible?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqPolicy}{Policy}
{How can a fault enter the inverse problem without becoming a fake residual?}
...
\mqPair{mqGate}{Gate}
{What is the main identifiability risk of fault-informed inversion?}
...
\mqPair{mqGate}{Gate}
{What final decision is needed before using faults quantitatively?}
...
\mqSeparator{Additional granular questions for OGS grounding}
\mqPair{mqGate}{Gate}
{Is atmospheric pressure available as a project time series?}
...
\mqPair{mqGate}{Gate}
{Is tunnel temperature available as a project time series?}
...
\mqPair{mqGate}{Gate}
{Is evapometer conductivity available as a project time series?}
...
\mqSeparator{Inverse-problem computation check}
\mqPair{mqPolicy}{Policy}
{What triage rule decides whether a newly found stream is usable?}
...
\mqPair{mqGate}{Gate}
{What should happen if a stream has values but no covariance?}
...
\mqPair{mqGate}{Gate}
{What source and processing record is required when a stream becomes active?}
